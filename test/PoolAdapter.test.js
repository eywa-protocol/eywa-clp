...
contract("StableSwap", () => {
  ...
  describe("Synthesis unit tests", () => {
    ...
    beforeEach(async () => {
      ...
      factory = poolType == "crypto" ? await ethers.getContractFactory("CryptoPoolAdapter") : await ethers.getContractFactory("StablePoolAdapter");
      ...
      await treasury.addTrustedAddress(curveBalancer.address);
    ...
    it("PoolAdapter: should exchange tokens", async function () {
      ...
      await poolAdapter.swap(tokens[0].address, pool.address, 0, 1, tokens[1].address, owner.address, 0, owner.address);
